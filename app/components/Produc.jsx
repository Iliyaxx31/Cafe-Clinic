...
const Produc = ({ img, text, title, price, onQuantityChange, initialQuantity = 0 }) => {
  ...
  return (
    <motion.div 
      ...
      whileHover={{ y: -4 }}
      className="group bg-gradient-to-t backdrop-blur-lg to-slate-400/25 from-gray-700/80 shadow-[0_0_9px_slate] rounded-2xl p-1.5 sm:p-2 hover:-translate-y-2 duration-300 border border-white/10 h-full flex flex-col"
    >
      <motion.div 
      ...
      <div className="flex items-center justify-between gap-2 px-1 py-1.5 mt-auto">
        ...
        <div className="flex items-center gap-[clamp(0.2rem,0.6vw,0.4rem)] shrink-0">
          <motion.button
            ...
            onClick={increase}
            aria-label="Arttır"
            className="shrink-0 aspect-square w-6 h-6 bg-gray-100/90 hover:bg-blue-500 hover:text-white duration-200 text-black rounded-full transition flex items-center justify-center text-sm"
          >
            <BsPlus size="1em" />
          </motion.button>
          ...
          <motion.span 
            ...
            className="text-center text-red-100 font-bold text-sm shrink-0 min-w-[1rem]"
          >
            {quantity}
          </motion.span>
          ...
          <motion.button
            ...
            onClick={decrease}
            aria-label="Azalt"
            className="shrink-0 aspect-square w-6 h-6 bg-white hover:bg-red-500 hover:text-white duration-200 text-gray-700 rounded-full transition flex items-center justify-center text-sm"
          >
            <BsDash size="1em" />
          </motion.button>
        </div>
